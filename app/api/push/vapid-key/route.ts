 ...
 * no IndexedDB copy) to hand it the key. Middleware whitelists exactly this
 * path (session cookies can be absent during pushsubscriptionchange), which
 * exposes nothing: /api/push/subscribe still requires the session, so a
 * signed-out worker could not register a row even with the key.
 */
